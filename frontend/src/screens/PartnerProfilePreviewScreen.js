...
export default function PartnerProfilePreviewScreen({ partner = {}, onBack }) {
  ...
  const partnerCategory = partner.partnerCategory || 'TCM Partner Institute';
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        message: `Check out ${instituteName} on TCM App! Accredited IT lab access & courses in ${location}.`
      });
    } catch (e) {}
  };
          ...
          <View style={styles.logoBadgeRow}>
            ...
            <View style={styles.badgesCluster}>
              ...
              <View style={[styles.statusTagPill, { backgroundColor: '#F0F5FF' }]}>
                ...
                <Text style={[styles.statusTagText, { color: '#2563EB' }]}>TCM Verified Partner</Text>
              </View>
            </View>
          </View>
