...
export default function WalletScreen({ session, user = {}, onBack }) {
  ...
  function handleCopyReferral() {
    ...
    Alert.alert("Referral Code Copied", `Code: ${referralCode}\nShare with friends to earn ₹100 & 50 TCM One Coins per referral!`);
  }

  function handleShareReferral() {
    Alert.alert(
      "Share Referral Link",
      `Join me on TCM One Academy using my code ${referralCode} and get 10 TCM One Coins & ₹500 converted referral reward!\n\nLink: https://tcm.app/ref/${referralCode}`
    );
  }
  ...
  async function handleConvertCoins() {
    if (walletData.tcmCoins < 500) {
      Alert.alert(
        "Coins Requirement",
        `You currently have ${walletData.tcmCoins} TCM One Coins. You need 500 coins to convert to ₹100 cash (500 Coins = ₹100).\n\nApply a referral code (+20 coins) or invite friends to earn coins!`
      );
      return;
    }
    ...
    try {
        ...
        transactions: [
          {
            ...
            subtitle: `Converted ${coinsDeducted} TCM One Coins to ₹${cashAdded.toFixed(2)} Cash`,
      ...
      Alert.alert("Coins Converted! 🎉", `${coinsDeducted} TCM One Coins converted successfully! ₹${cashAdded.toFixed(2)} cash added to your available balance.`);
    } catch (err) {
  ...
  function handleWithdrawWhatsApp() {
    ...
    const msg = `Hi TCM One Support! I want to withdraw ₹${amt.toFixed(2)} from my TCM One Wallet. My UPI / Bank details: ${upi}. Please process my withdrawal.`;
  ...
  async function handleAddMoneySubmit() {
    ...
    try {
      ...
      setAddAmount("");
      Alert.alert("Money Added Successfully", `₹${amt.toFixed(2)} added to your TCM One Wallet balance.`);
    } catch (err) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <View style={[styles.topHeader, { backgroundColor: theme.bg, borderColor: theme.border }]}>
        ...
        <Pressable onPress={() => Alert.alert("TCM One Wallet Help", "Earn coins via referrals and convert coins to withdrawable cash anytime!")} style={styles.helpBtn}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.metricsCard, { backgroundColor: theme.cardBg, borderColor: theme.border }]}>
          ...
          {/* Column 3: TCM One Coins */}
          <View style={styles.metricCol}>
            <Text style={[styles.metricLabel, { color: theme.subtext }]}>TCM One Coins</Text>
            <View style={styles.metricValueRow}>
        ...
        <View style={[styles.referralCard, { backgroundColor: theme.isDark ? theme.badgeBg : "#F0FDFA", borderColor: theme.border }]}>
          ...
          <View style={{ marginTop: 14, paddingTop: 14, borderTopWidth: 1, borderTopColor: theme.border }}>
            ...
            <Text style={{ fontFamily: fonts.regular, fontSize: 11.5, color: theme.subtext, marginBottom: 8 }}>Enter someone's referral code to get 20 TCM One Coins instantly!</Text>
        ...
        <View style={[styles.coinConvCard, { backgroundColor: theme.cardBg, borderColor: theme.border }]}>
          ...
          <View style={{ marginTop: 14, backgroundColor: theme.badgeBg, borderRadius: 14, padding: 12, borderWidth: 1, borderColor: theme.border }}>
            ...
            <Text style={{ fontFamily: fonts.regular, fontSize: 11.5, color: theme.text, lineHeight: 17 }}>
              Aap apne converted wallet cash (<Text style={{ fontFamily: fonts.bold, color: theme.primary }}>₹{walletData.availableBalance.toFixed(2)}</Text>) se naye TCM One Courses purchase kar sakte hain ya instant course discount claim kar sakte hain!
            </Text>
          </View>
        </View>
        ...
        <View style={styles.stepsGrid}>
          ...
          <View style={[styles.stepCard, { backgroundColor: theme.cardBg, borderColor: theme.border }]}>
            ...
            <Text style={[styles.stepSubText, { color: theme.subtext }]}>Invite your friends to TCM One Academy</Text>
          </View>
