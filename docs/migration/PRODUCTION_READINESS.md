...
- The exposed Atlas runtime credential was replaced without downtime. Production now uses `tcm_app_prod` with `readWrite` limited to `tcm_ac`; the former shared `atlasAdmin` database user was revoked. A fresh post-rotation backup was checksum-verified on the backend, sentinel and OCI Object Storage.
...
6. **Authentication smoke test:** validate migrated PHP bcrypt (`$2y$`) users with designated non-privileged test accounts and verify password-reset fallback.
7. **Ownership:** name the cutover commander, Hostinger owner, OCI owner and product acceptance owner for the change window.
