...
export default function AiRoadmapPlannerModal({ visible, onClose, user = {}, courses = [], mentors = [] }) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `ai_err_${Date.now()}`,
          sender: "ai",
          text: "I've processed your learning request! Based on your goal, we recommend starting with LastClass Full Stack Web Masterclass or AI Specialization. Tap below to export your roadmap to WhatsApp!",
          time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
        }
      ]);
    } finally {
  ...
  function handleSendToWhatsApp() {
    ...
    const waMsg =
      ...
      `----------------------------------------\n` +
      `📲 *Generated by Oveta AI (LastClass)*\n` +
