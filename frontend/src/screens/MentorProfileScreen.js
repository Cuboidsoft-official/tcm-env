...
export default function MentorProfileScreen({ session, user = {}, targetMentor = null, mentorId, onClose, onOpenCourseDetails, onOpenChat, onEditCourse, onSelectPost }) {
  ...
  const mentorName = rawActive.name || rawActive.fullName || (isTargetUser ? user.name : "") || "TCM One Certified Mentor";
  ...
  const mentorBio = rawActive.bio || rawActive.about || rawActive.description || (isTargetUser ? user.bio : "") || `${mentorName} is a verified TCM One mentor specializing in ${mentorRole.toLowerCase()} and hands-on student mentoring.`;
    ...
    initialExpList = [
      {
        id: "exp1",
        role: mentorRole,
        company: `${rawActive.organization || rawActive.company || "TCM One Educator Network"} • Active Instructor`,
  ...
  const data = {
    ...
    tags: rawActive.tags || [
      ...
      { label: "TCM One Educator", bg: "#ECF9E9", color: "#2E7D32" }
    ],
    bio: mentorBio,
  ...
  function handleBookSessionWhatsApp() {
    ...
    const roleStr = data.role || "TCM One Educator";
              ...
              >
                ...
                <View style={styles.mentorCourseContent}>
                  <View style={[styles.badgePillSmall, { backgroundColor: theme.badgeBg }]}>
                    <Text style={[styles.badgePillSmallText, { color: theme.primary }]}>{course.category || "TCM One Course"}</Text>
                  </View>
          ...
          <View style={[styles.featuredReviewCard, themedSoftSurface]}>
            <View style={styles.reviewAuthorRow}>
              ...
              <View style={styles.reviewAuthorWrap}>
                <Text style={[styles.reviewAuthorName, { color: theme.text }]}>{featuredReview.authorName || "TCM One Student"}</Text>
