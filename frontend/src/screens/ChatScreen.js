...
export default function ChatScreen({ session, user = {}, targetUser: initialTargetUser, targetUserId = "m1", onClose, onDeleteChannel, onOpenUserProfile }) {
    ...
    try {
      if (fullscreenImageUri) {
        await Share.share({
          ...
          message: `Check out this photo from TCM One Chat: ${fullscreenImageTitle || "Photo Attachment"}`,
  ...
  const fallbackTargetUser = {
    ...
    name: initialTargetUser?.name || "TCM One Member",
    role: initialTargetUser?.role || "TCM One Mentor & Member",
        ...
        <Pressable onPress={() => setShowDetailsModal(true)} style={styles.headerUserCol}>
          ...
          <View style={styles.headerTextWrap}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", marginTop: 1 }}>
              ...
              <Text numberOfLines={1} style={[styles.headerStatus, { color: theme.subtext }]}>Online  •  {currentTarget.role || "TCM One Member"}</Text>
            </View>
          </View>
        </Pressable>
            ...
            return (
              <View key={String(msg.id || `msg_${index}`)} style={[styles.channelPostCard, { backgroundColor: theme.cardBg, borderColor: theme.border }]}>
                ...
                <View style={[styles.channelPostFooter, { borderTopColor: theme.border, justifyContent: "flex-start", gap: 12 }]}>
                  ...
                  <TouchableOpacity
                    onPress={async () => {
                      const shareMsgText = `${currentTarget.name || "TCM One Channel"}: ${msg.text || "Check out this update on TCM One"}`;
                      try {
                        await Share.share({
                          title: currentTarget.name || "TCM One Channel Update",
